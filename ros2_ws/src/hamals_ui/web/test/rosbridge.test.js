...
test('camera command follows /fork/is_up feedback and needs a live connection', t => {
  const f = setup(t)
  assert.equal(f.bridge.cameraIsBack.value, false)
  assert.equal(f.bridge.sendCmd({ type: 'switch_camera', payload: { is_up: true } }), false)
  f.connect()
  const camera = f.topics.find(x => x.name === '/fork/is_up' && x.cb)
  const publisher = f.topics.find(x => x.name === '/fork/is_up' && !x.cb)
  assert.equal(publisher.messageType, 'std_msgs/msg/Bool')
  assert.equal(f.bridge.sendCmd({ type: 'switch_camera', payload: { is_up: true } }), true)
  assert.equal(f.sent.at(-1).topic, '/fork/is_up')
  assert.equal(f.sent.at(-1).msg.data, true)
  assert.equal(f.bridge.cameraIsBack.value, true)
  assert.equal(f.bridge.sendCmd({ type: 'switch_camera', payload: { is_up: false } }), true)
  assert.equal(f.sent.at(-1).topic, '/fork/is_up')
  assert.equal(f.sent.at(-1).msg.data, false)
  assert.equal(f.bridge.cameraIsBack.value, false)
  assert.equal(f.sent.some(x => x.topic === '/ui/cmd'), false)
  camera.cb({ data: true })
  assert.equal(f.bridge.cameraIsBack.value, true)
  f.clients[0].close()
  assert.equal(f.bridge.sendCmd({ type: 'switch_camera', payload: { is_up: false } }), false)
  camera.cb?.({ data: false })
  assert.equal(f.bridge.cameraIsBack.value, true)
