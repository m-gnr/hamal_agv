...
test('camera command follows /fork/is_up feedback and needs a live connection', t => {
  ...
  f.connect()
  const camera = f.topics.find(x => x.name === '/fork/is_up')
  assert.equal(camera.messageType, 'std_msgs/msg/Bool')
  camera.cb({ data: true })
  assert.equal(f.bridge.cameraIsBack.value, true)
  assert.equal(f.bridge.sendCmd({ type: 'switch_camera', payload: { is_up: false } }), true)
  assert.equal(f.bridge.cameraIsBack.value, false)
  assert.deepEqual(JSON.parse(f.sent.at(-1).msg.data), { type: 'switch_camera', payload: { is_up: false } })
  assert.equal(f.sent.at(-1).topic, '/ui/cmd')
  f.clients[0].close()
  camera.cb?.({ data: true })
  assert.equal(f.bridge.cameraIsBack.value, false)
})
