...
def declare_parameters(node: Node) -> None:
    ...
    node.declare_parameter(
        'imu_ang_vel_cov_diag',
        [9999.0, 9999.0, 0.005]
    )

    node.declare_parameter(
        'imu_lin_acc_cov_diag',
        [0.01, 9999.0, 9999.0]
    )

    node.declare_parameter(
        'publish_linear_accel',
        True
    )
...
def load_config(node: Node) -> SerialBridgeConfig:
    ...
    cfg = SerialBridgeConfig(
        ...
        baudrate=int(
            node.get_parameter('baudrate').value
        ),

        timeout_ms=int(
            node.get_parameter('timeout_ms').value
        ),
        ...
        imu_ang_vel_cov_diag=list(
            node.get_parameter(
                'imu_ang_vel_cov_diag'
            ).value
        ),

        imu_lin_acc_cov_diag=list(
            node.get_parameter(
                'imu_lin_acc_cov_diag'
            ).value
        ),
...
def validate_config(cfg: SerialBridgeConfig) -> None:
    ...
    if len(cfg.imu_ang_vel_cov_diag) != 3:
        raise ValueError(
            "imu_ang_vel_cov_diag must contain 3 elements"
        )

    if len(cfg.imu_lin_acc_cov_diag) != 3:
        raise ValueError(
            "imu_lin_acc_cov_diag must contain 3 elements"
        )
