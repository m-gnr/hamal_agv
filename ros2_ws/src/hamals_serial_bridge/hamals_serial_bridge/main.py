...
class SerialBridgeNode(Node):
    ...
    def __init__(self):
        ...
        if self.cfg.debug:
            self.create_timer(
                1.0,
                self._print_debug_panel
            )

        self.create_timer(
            0.05,
            self._check_cmd_timeout
        )
        ...
        self.get_logger().info(
            format_startup_config(self.cfg)
        )

        self.get_logger().info(
            "hamals_serial_bridge started"
        )
    ...
    def _reset_mcu(self):
        ...
        self.get_logger().info(
            "Resetting MCU via DTR"
        )

        self.ser.dtr = False

        time.sleep(
            self.cfg.reset_pulse_ms / 1000.0
        )

        self.ser.dtr = True

        time.sleep(
            self.cfg.reset_boot_wait_ms / 1000.0
        )
    ...
    def _maybe_send_cmd(
        ...
        if (
            self._last_sent_cmd_v is None
            or
            self._last_sent_cmd_w is None
        ):
        ...
        new_stop = self._is_stop_cmd(
            v,
            w
        )

        # Stop <-> movement
        # geçişleri rate-limit'e takılmasın
    ...
    def _cmd_changed(
        ...
        dv = abs(
            v - self._last_sent_cmd_v
        )

        dw = abs(
            w - self._last_sent_cmd_w
        )

        return (
            dv > self.cfg.cmd_dedup_eps_v
            or
            dw > self.cfg.cmd_dedup_eps_w
        )
    ...
    def fork_cmd_callback(
        ...
        if cmd not in (
            ...
            self.get_logger().warn(
                f"Invalid fork command: {cmd}"
            )

            return

        frame = encode_fork_cmd(
            cmd
        ).encode('utf-8')
    ...
    def _update_rx_debug_stats(self):
        ...
        self._dbg_rx_invalid = (
            self.parser.invalid_frames
        )
    ...
    def _publish_fork_state(
        ...
        out.error_code = int(
            msg.get(
                'error_code',
                ForkState.ERROR_NONE
            )
        )

        out.upper_limit = bool(
            msg.get(
                'upper_limit',
                False
            )
    ...
    def _publish_imu(
        ...
        imu_msg.angular_velocity_covariance = [
            float(
                self.cfg.imu_ang_vel_cov_diag[0]
            ),
            0.0,
            0.0,
            ...
            0.0,
            float(
                self.cfg.imu_ang_vel_cov_diag[1]
            ),
            0.0,
            ...
            0.0,
            float(
                self.cfg.imu_ang_vel_cov_diag[2]
            ),
        ]
        ...
        imu_msg.linear_acceleration_covariance = [
            float(
                self.cfg.imu_lin_acc_cov_diag[0]
            ),
            0.0,
            0.0,
            ...
            0.0,
            float(
                self.cfg.imu_lin_acc_cov_diag[1]
            ),
            0.0,
            ...
            0.0,
            float(
                self.cfg.imu_lin_acc_cov_diag[2]
            ),
        ]
    ...
    def _print_debug_panel(self):
        ...
        panel = format_debug_panel(
            ...
            tx_dedup_skipped=(
                self._dbg_tx_skipped_dedup
            ),

            tx_ratelimit_skipped=(
                self._dbg_tx_skipped_ratelimit
            ),

            wheel_ticks_published=(
                self._dbg_wheel_ticks_tx
            ),
